...
//!
//! The set mirrors the original keel's icon usage exactly:
...
//!   are ports of the hand-drawn inline SVGs in keel's `terminal-panel.tsx` /
//!   `composer-actions.tsx`.
//! - The harness brand marks (`claude-mark`, `openai-mark`, `cursor-mark`) are
//!   ports of keel's `icons.tsx`. gpui tints SVGs with the text color, so the
...
icon_assets![
    ...
    // Mirrored variant (keel window-controls.tsx `-scale-x-100`): the LEFT
    ...
    // Hand-drawn keel glyphs (terminal-panel.tsx / composer-actions.tsx /
