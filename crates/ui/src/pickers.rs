...
use keel_engine::registry::HarnessDescriptor;
use keel_proto::{
    ChatConfig, FolderListing, HarnessId, Model, ModelOption, ReasoningLevel, RepoRef,
    SandboxLevel, Space,
};
...
pub enum CheckoutPlan {
    ...
    /// `CreateWorktree` off `base` on send (keel mints a `keel/<name>`
...
/// lead with the flagship — keel's `pickDefaultModel` Opus preference maps to
/// the same row here).
pub fn default_model(models: &[Model]) -> Option<&Model> {
    models.first()
}

/// A model's default reasoning: X-High when the ladder offers it (keel
...
/// or foreign level — keel use-run-config.ts's derived-model discipline).
...
fn supported_reasoning(harness: HarnessId, level: ReasoningLevel) -> bool {
...
/// The fallback Codex catalog advertises `serviceTier`, but the local ACP
/// adapter has no matching config option to apply it. Live `fastMode` options
/// still appear because the adapter sends those through by their real IDs.
fn supported_option(option: &ModelOption) -> bool {
    !option.id.eq_ignore_ascii_case("serviceTier")
}
...
/// Only present native-style toggles for a two-choice capability the local
/// agent actually advertised. Every other option keeps the generic row UI.
fn binary_option(option: &ModelOption) -> Option<BinaryOption> {
    ...
    let id: String = option
        ...
        .collect();
    let choice = |wanted: &[&str]| {
        option.choices.iter().any(|choice| {
            wanted.iter().any(|word| {
                choice.id.eq_ignore_ascii_case(word) || choice.label.eq_ignore_ascii_case(word)
            })
        })
    };
    if matches!(id.as_str(), "collaborationmode" | "mode")
        && choice(&["plan"])
        && choice(&["default", "agent"])
    {
        Some(BinaryOption::Mode)
    } else if matches!(id.as_str(), "fastmode" | "speedmode")
        && choice(&["on", "fast"])
        && choice(&["off", "standard", "default"])
    {
        Some(BinaryOption::Fast)
    } else {
        None
    }
}

fn binary_choice_label(kind: BinaryOption, choice: &keel_proto::ModelOptionChoice) -> &'static str {
    match kind {
        BinaryOption::Mode => {
            if choice.id.eq_ignore_ascii_case("plan") || choice.label.eq_ignore_ascii_case("plan") {
                "Plan"
            } else {
                "Default"
            }
        ...
        BinaryOption::Fast => {
            if choice.id.eq_ignore_ascii_case("on")
                || choice.id.eq_ignore_ascii_case("fast")
                || choice.label.eq_ignore_ascii_case("fast")
            {
                "On"
            } else {
                "Off"
...
fn persist_option_choice(
    selections: &mut serde_json::Map<String, serde_json::Value>,
    option_id: String,
    choice_id: String,
    default: bool,
) {
    let id: String = option_id
        .chars()
        .filter(|c| c.is_ascii_alphanumeric())
        .flat_map(char::to_lowercase)
        .collect();
    // Explicit defaults reset a resumed local ACP session that was previously
    // in Plan or Fast mode; absence would leave its current value untouched.
    if default && !matches!(id.as_str(), "collaborationmode" | "fastmode") {
        selections.remove(&option_id);
    } else {
        selections.insert(option_id, serde_json::Value::String(choice_id));
    }
}
...
pub fn traits_summary(
    ...
    if let Some(model) = model {
        for option in model.options.iter().filter(|o| supported_option(o)) {
...
pub fn browser_rows(listing: &FolderListing) -> Vec<&keel_proto::FolderEntry> {
...
pub struct Pickers {
    ...
    config: DraftConfig,
    /// Sticky last-used picks (keel `keel.composer.defaults:v1`): seeds the
    ...
    focus: FocusHandle,
    /// `KEEL_OPEN_PICKER` boot: keep claiming focus until it sticks, so
...
impl Pickers {
    pub fn new(state: Entity<AppState>, cx: &mut Context<Self>) -> Self {
        ...
        // Dev/testing knob: `KEEL_OPEN_PICKER=model|traits|repo|branch` boots
        // with that popover open — synthetic input can't reach the app on
        // headless compositors, so captures need a data-side path.
        let boot_open = match std::env::var("KEEL_OPEN_PICKER").ok().as_deref() {
    ...
    fn effective_harness(&self, cx: &App) -> Option<HarnessId> {
        ...
        // model (it stays available under `KEEL_HARNESS=mock`).
        self.harnesses
            .ready()
    ...
    pub fn resolved_steering_mode(&self, cx: &App) -> Option<keel_proto::SteeringMode> {
        let harness = self.effective_harness(cx)?;
        self.harnesses
            .ready()
    ...
    /// Auto-route only an unpinned new session, or a session that already
    /// opted into Jev. A manual model pick removes that opt-in below.
    pub fn jev_auto_eligible(&self, cx: &App) -> bool {
        if let Some(chat) = self.state.read(cx).selected_chat_row() {
            return chat.config.as_ref().is_some_and(|config| {
                config.model_options.get("jevAuto") == Some(&serde_json::Value::Bool(true))
            });
        }
        self.config.model.is_none()
            && self
                .effective_harness(cx)
                .and_then(|harness| self.defaults.model_for(harness))
                .is_none()
    }
    ...
    /// Capture knob (`KEEL_OPEN_DIALOG=model`): open the combined
    ...
    fn pick_option(
        ...
        if self.state.read(cx).selected_chat.is_some() {
            self.update_chat_config(cx, move |config| {
                persist_option_choice(&mut config.model_options, option_id, choice_id, default);
            });
        } else {
            persist_option_choice(
                &mut self.config.model_options,
                option_id,
                choice_id,
                default,
            );
    ...
    fn trait_ladder(&self, cx: &App) -> Vec<ReasoningLevel> {
        ...
        levels
            .into_iter()
            .filter(|level| harness.is_none_or(|h| supported_reasoning(h, *level)))
            .collect()
    }
    ...
    fn device_rows(&self, cx: &App) -> Vec<keel_proto::Device> {
        let state = self.state.read(cx);
        let local = state.local_device_id.clone();
        let mut devices: Vec<keel_proto::Device> = state.devices.clone();
    ...
    fn filtered_device_rows(&self, cx: &App) -> Vec<keel_proto::Device> {
        let query = self.search.read(cx).text().to_string();
    ...
    fn trigger_chip(
        ...
        // Ghost pill (keel composer/styles.tsx `pill`): `h-8 rounded-lg px-2.5
        ...
        div()
            ...
            // keel composer/styles.tsx `pill`: `transition-colors` — the wash
    ...
    fn popover_frame(&self, width: f32, content: AnyElement, cx: &mut Context<Self>) -> AnyElement {
        ...
        popover::popover_card(&theme)
            .w(px(width))
            // keel caps its tallest picker at min(640px, 75vh).
            .max_h(px(640.0))
    ...
    /// picker's rail + list panes bleed to the card edge (keel
    ...
    fn render_traits_sections(&mut self, cx: &mut Context<Self>) -> AnyElement {
        ...
        if !levels.is_empty() {
            let active_index = levels.iter().position(|level| Some(*level) == current);
            sections.push(
                div()
                    .flex()
                    .flex_col()
                    ...
                    .child(
                        div()
                            ...
                            .children(levels.into_iter().enumerate().map(|(ix, level)| {
                                ...
                                let reached = active_index.is_some_and(|active| ix <= active);
                                div()
                                    .id(("reasoning-step", ix))
                                    .flex_1()
                                    .min_w_0()
                                    .py(px(4.0))
                                    .flex()
                                    ...
                                    .child(div().w_full().h(px(6.0)).rounded_full().bg(
                                        if reached {
                                            theme.accent
                                        } else {
            ...
            .enumerate()
        {
            ...
            if let Some(kind) = binary_option(option) {
                let label = match kind {
                    BinaryOption::Mode => "Collaboration mode",
                    BinaryOption::Fast => "Fast mode",
                };
                sections.push(
                    div()
                        .flex()
                            ...
                            div().flex().gap(px(4.0)).children(
                                option
                                    ...
                                    .map(|(choice_ix, choice)| {
                                        ...
                                        div()
                                            .id(("trait-toggle", opt_ix * 2 + choice_ix))
                                            .flex_1()
                                            .min_w_0()
                                            .h(px(30.0))
                                            .flex()
                                            ...
                                            .child(binary_choice_label(kind, choice))
                                    }),
                            ),
                        )
                        .into_any_element(),
...
pub(crate) fn normalize_model_rows(harness: HarnessId, models: Vec<Model>) -> Vec<Model> {
    ...
    let catalog = match harness {
        HarnessId::ClaudeCode => keel_harness::claude::catalog::static_models(),
        _ => Vec::new(),
    };
        ...
        .filter_map(|mut model| {
            ...
            if let Some(base) = strip_1m(&model.id.clone()) {
                ...
                if !model.options.iter().any(|o| o.id == "contextWindow") {
                    model.options.push(keel_proto::ModelOption {
                        id: "contextWindow".into(),
                        label: "Context Window".into(),
                        choices: vec![
                            keel_proto::ModelOptionChoice {
                                id: "200k".into(),
                                label: "200K".into(),
                            },
                            keel_proto::ModelOptionChoice {
...
/// `KEEL_HARNESS=mock` (the e2e/dev rig) opts the mock harness into the UI;
/// production launches never set it, so the mock never surfaces there.
fn mock_harness_enabled() -> bool {
    std::env::var("KEEL_HARNESS").ok().as_deref().map(str::trim) == Some("mock")
}
...
/// `KEEL_HARNESS=mock` shows it; otherwise it only remains when it's
...
fn offered_harnesses_impl(list: &[HarnessDescriptor], allow_mock: bool) -> Vec<HarnessDescriptor> {
    ...
    let offered: Vec<HarnessDescriptor> = visible
        .iter()
        .filter(|d| {
            keel_engine::registry::descriptor_enabled(d) || (allow_mock && d.id == HarnessId::Mock)
        })
        .cloned()
        .collect();
...
impl Render for Pickers {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        let theme = Theme::of(cx).clone();
        // A KEEL_OPEN_PICKER popover never went through `toggle`, so claim
        ...
        self.prefetch_models(cx);
        // A popover opened data-side (KEEL_OPEN_PICKER) never went through
        ...
        // Chip shows the model's display name alone (keel `modelText`); the
        ...
        // attach + send after this element (keel composer-actions.tsx
        // arrangement).
        let left = div()
            .flex()
            .flex_row()
        ...
        let has_traits = !self.trait_ladder(cx).is_empty()
            || self
                .selected_model(cx)
                .is_some_and(|m| m.options.iter().any(supported_option));
...
mod tests {
    use super::*;
    use keel_proto::{FolderEntry, Model, ModelOption, ModelOptionChoice};
    ...
    #[test]
    fn native_controls_require_advertised_binary_capabilities() {
        let option = |id: &str, left: &str, right: &str| ModelOption {
            id: id.into(),
            label: id.into(),
            choices: vec![
                ModelOptionChoice {
            ...
            default_choice: left.into(),
        };
        let mode = option("collaboration_mode", "default", "plan");
        let fast = option("fast-mode", "off", "on");
        let static_tier = option("serviceTier", "default", "fast");
        assert_eq!(binary_option(&mode), Some(BinaryOption::Mode));
        assert_eq!(binary_option(&fast), Some(BinaryOption::Fast));
        assert_eq!(
            binary_choice_label(BinaryOption::Mode, &mode.choices[0]),
            "Default"
        );
        assert_eq!(
            binary_choice_label(BinaryOption::Fast, &fast.choices[1]),
            "On"
        );
        assert!(!supported_option(&static_tier));
        assert_eq!(binary_option(&static_tier), None);
        assert_eq!(binary_option(&option("thinking", "off", "on")), None);
        assert!(!supported_reasoning(
            HarnessId::Codex,
            ReasoningLevel::Minimal
        ));
        assert!(supported_reasoning(HarnessId::Codex, ReasoningLevel::Ultra));
        assert!(supported_reasoning(
    ...
    #[test]
    fn default_and_off_are_explicit_for_resumed_agent_sessions() {
        ...
        persist_option_choice(&mut selections, "fast-mode".into(), "off".into(), true);
        assert_eq!(
            selections.get("collaboration_mode"),
            Some(&serde_json::json!("default"))
        );
        assert_eq!(selections.get("fast-mode"), Some(&serde_json::json!("off")));
        persist_option_choice(
            &mut selections,
            "contextWindow".into(),
            "standard".into(),
        ...
        assert!(!selections.contains_key("contextWindow"));
    }
    ...
    fn completion_prefix_lengths() {
        ...
        assert_eq!(completion_prefix_len("keel", "keel"), Some(4));
        assert_eq!(completion_prefix_len("keel", ""), Some(0));
        assert_eq!(completion_prefix_len("keel", "dev"), None);
    ...
    fn browser_navigation_reducer() {
        ...
        assert_eq!(browser_rows(&listing)[1].name, "keel");
    }
    ...
    fn mock_harness_hidden_unless_alone() {
        let descriptor = |id: HarnessId, name: &str| HarnessDescriptor {
            ...
            steering_mode: keel_proto::SteeringMode::StepBoundary,
            reasoning_levels: vec![],
            installed: true,
            enabled: None,
        };
        ...
        // …and opted back in by KEEL_HARNESS=mock (the e2e rig).
