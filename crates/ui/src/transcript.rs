...
use keel_doc::{MessagePart, MessageRole, MessageStatus, SessionMessageEntry};
use keel_proto::{
    DecisionActivity, DecisionBackend, DecisionEvent, DecisionPhase, DecisionResult, DecisionStage,
    DecisionValidation, ToolCall,
};
...
use keel_syntax::LanguageId as Lang;
...
pub enum ToolDetail {
    ...
    Stats {
        stats: Arc<Vec<keel_doc::ToolDiffStat>>,
    },
}
...
pub fn tool_detail(
    output: Option<&str>,
    diff: Option<&keel_proto::ToolDiff>,
    diff_stats: Option<&[keel_doc::ToolDiffStat]>,
) -> Option<ToolDetail> {
...
/// Reduce an inline [`keel_proto::ToolDiff`] to the changes pane's
...
pub fn diff_to_file(diff: &keel_proto::ToolDiff) -> crate::changes::FileDiff {
...
pub struct Row {
    ...
    /// its timestamp strip (keel chat-view.tsx `group`/`group-hover`).
        ...
        match part {
            ...
            other => {
                ...
                match other {
                    ...
                    MessagePart::Decision { id: part_id, event } => {
                        let event = event.clone().bounded();
                        let encoded = serde_json::to_vec(&event).unwrap_or_default();
                        rows.push(Row {
                            id: format!("{}#{}", entry.id, part_id).into(),
                            version: fnv1a(&encoded),
                            turn_start: false,
...
/// `KEEL_FRAME_STATS=1` logs live-row render-cost percentiles (p50/p95 µs
...
fn frame_stats_enabled() -> bool {
    ...
    *ENABLED
        .get_or_init(|| std::env::var("KEEL_FRAME_STATS").is_ok_and(|v| !v.is_empty() && v != "0"))
}

const FRAME_STATS_WINDOW: usize = 240;

/// `KEEL_NO_RENDER_CACHE=1` bypasses the cross-frame flatten cache — the
/// A/B knob for the frame-cost measurement above.
fn render_cache_disabled() -> bool {
    static DISABLED: std::sync::OnceLock<bool> = std::sync::OnceLock::new();
    *DISABLED.get_or_init(|| {
        std::env::var("KEEL_NO_RENDER_CACHE").is_ok_and(|v| !v.is_empty() && v != "0")
    })
}
...
///
/// The rule lives in `keel_proto::view` so the terminal viewport reports the
...
pub fn tool_group_summary(tools: &[ToolItem]) -> String {
    ...
    keel_proto::view::tool_group_summary(&pairs)
}

// `single_line` and the per-kind chip label/detail are shared with the terminal
// viewport (`keel_proto::view`): a tool must be named identically on every
...
// cell grid).
pub use keel_proto::view::{single_line, tool_chip_content};
...
fn blob_detail(text: &str, is_diff: bool) -> Option<ToolDetail> {
    if is_diff {
        let diff: keel_proto::ToolDiff = serde_json::from_str(text).ok()?;
...
/// Selector activity is named only from a fresh engine session row. The
...
fn decision_backend_label(backend: DecisionBackend) -> &'static str {
    match backend {
...
fn decision_stage_label(stage: DecisionStage) -> &'static str {
    match stage {
        DecisionStage::Intake => "Intake",
        DecisionStage::PreStep => "Pre-step",
        DecisionStage::Context => "Context",
        DecisionStage::ToolBoundary => "Tool boundary",
        DecisionStage::Completion => "Completion",
    }
}

fn decision_validation_label(validation: DecisionValidation) -> &'static str {
    match validation {
        DecisionValidation::Accepted => "validated",
        DecisionValidation::Rejected => "rejected",
        DecisionValidation::Stale => "stale",
        DecisionValidation::Expired => "expired",
        DecisionValidation::Unauthorized => "unauthorized",
    }
}

/// Pure factual projection used by both the card and tests. No model trace or
/// inferred explanation is synthesized here.
fn decision_event_lines(event: &DecisionEvent) -> Vec<String> {
    let mut lines = Vec::with_capacity(event.candidates.len() + 4);
    lines.push(format!(
        "{} · {} · revision {}",
        decision_backend_label(event.backend),
        decision_stage_label(event.stage),
        event.task_revision
    ));
    lines.extend(
        event
            .candidates
            .iter()
            .map(|candidate| format!("Considered · {} · {}", candidate.id, candidate.summary)),
    );
    lines.push(match &event.result {
        DecisionResult::Selected { candidate_id } => format!(
            "Selected · {candidate_id} · {}",
            decision_validation_label(event.validation)
        ),
        DecisionResult::Abstained => format!(
            "Abstained · {}",
            decision_validation_label(event.validation)
        ),
    });
    ...
    if let Some(probability) = event.selected_probability {
        lines.push(format!(
            "Selected probability · {:.0}%",
            probability * 100.0
        ));
    }
    if let Some(fit) = event.fit {
        lines.push(format!("Fit · {:.0}%", fit * 100.0));
    }
    if let Some(fallback) = &event.fallback {
        lines.push(format!("Fallback · {fallback}"));
    }
    if let Some(outcome) = &event.observed_outcome {
        lines.push(format!("Observed · {outcome}"));
    }
    lines
}

fn decision_event_card(event: &DecisionEvent, theme: &Theme) -> AnyElement {
    let lines = decision_event_lines(event);
    let mut card = div()
        .w_full()
    ...
    for (index, line) in lines.into_iter().enumerate() {
        card = card.child(
            div()
                .text_size(px(if index == 0 { 12.0 } else { 11.0 }))
...
struct HighlightEntry {
    ...
    document: Option<Weak<keel_syntax::HighlightedDocument>>,
    _task: Option<Task<()>>,
}
...
impl HighlightStore {
    ...
    fn request(
    ...
    ) -> Option<Arc<keel_syntax::HighlightedDocument>> {
        ...
        let task = cx.spawn(async move |this, cx| {
            ...
            let document = cx
                .background_executor()
                .spawn(async move {
                    keel_syntax::highlight(keel_syntax::HighlightRequest {
                        source: &code,
                        path: None,
...
pub struct Transcript {
    ...
    /// Only the freshly sent local message gets the brief bubble entrance.
    sent_entry_at: Option<(SharedString, Instant)>,
    ...
    /// strip (keel chat-view.tsx `group-hover`; the rows report hover
...
impl Transcript {
    ...
    pub fn on_own_send(&mut self, chat_id: String, message_id: String, cx: &mut Context<Self>) {
        self.sent_entry_at = Some((SharedString::from(message_id.clone()), Instant::now()));
        self.pinned = false;
    ...
    fn spawn_blob_fetch(&mut self, blob_ref: SharedString, cx: &mut Context<Self>) {
        ...
        let task = cx.spawn(async move |this, cx| {
            let reply = crate::attachments::call_with_timeout(
                ...
                keel_rpc::methods::FETCH_TOOL_BLOB,
    ...
    /// device (uploads targeted it) plus this device (keel's
    ...
    fn render_working_trailer(&self, cx: &mut Context<Self>) -> Option<AnyElement> {
        ...
        let (decision_activity, sending, elapsed_secs) = {
            let state = self.state.read(cx);
            let activity = state.decision_activity_for(&chat_id, now);
            if activity.is_none()
                && state.indicator_for(&chat_id, now) != crate::state::Indicator::Working
            {
                return None;
            }
            ...
            let elapsed = turn_started
                ...
                .unwrap_or(0);
            (activity, sending, elapsed)
        };
        let label = if let Some(activity) = decision_activity {
            decision_activity_label(activity)
        } else if sending {
            "Sending…"
        } else {
            // Existing neutral Working vocabulary, never a claim that a
            // decision model is evaluating the prompt.
            flavour_word(flavour_seed(&chat_id), elapsed_secs)
        };
        let label = if decision_activity.is_none() && !sending {
            format!("{label}…")
        } else {
            label.to_string()
        };
        let deciding = decision_activity.is_some();
        ...
        Some(
            div()
                ...
                .when(decision_activity.is_none() && !sending, |el| {
                    el.child(
                        div()
    ...
    fn render_row(&mut self, ix: usize, window: &mut Window, cx: &mut Context<Self>) -> AnyElement {
        ...
        let inner: AnyElement = match &row.kind {
            RowKind::User {
                ...
                if self.sent_entry_at.as_ref().is_some_and(|(id, started)| {
                    id == &row.id && started.elapsed() < motion::DIALOG_IN.total()
                }) && !motion::reduced_motion(cx)
                {
                    motion::message_send_in(SharedString::from(format!("sent-{}", row.id)), column)
                        .into_any_element()
                } else {
            ...
            RowKind::Decision { event } => decision_event_card(event, &theme),
        };

        // Hover-revealed timestamp strip (keel chat-view.tsx `Timestamp`):
        ...
        div()
            ...
            .pt(px(top_gap))
            .pb(px(bottom_pad))
            // Wide gutters (keel `px-4 @3xl:px-12`) around the 46rem column.
            .px(px(48.0))
            .child(
                div()
                    .w_full()
    ...
    fn code_highlight_for(
        ...
        only: Option<usize>,
        cx: &mut Context<Self>,
    ) -> HashMap<usize, Option<Arc<keel_syntax::HighlightedDocument>>> {
        ...
        for (ix, top) in tree.blocks.iter().enumerate() {
            ...
            if let Block::CodeBlock { language, code } = &top.block
                && let Some(lang) = language
                    .as_deref()
                    .and_then(keel_syntax::language_for_alias)
            {
                out.insert(
                    ix,
    ...
    fn tool_diff_highlight_for(
        ...
        let old = match old_text {
            Some(source) => {
                ...
                let lang = keel_syntax::language_for_path(path)?;
                Some(
                    self.highlights
        ...
        let new = match new_text {
            Some(source) => {
                let lang = keel_syntax::language_for_path(&file.path)?;
    ...
    fn render_tool_group(
        ...
        // Header (keel tool-group.tsx): a small chevron tile centered over the
        ...
        let header = div()
            ...
            // chips (destructive tint, keel tool-chip.tsx) and in the
...
/// The transcript ErrorChip — an exact port of keel chat-view.tsx
...
/// A small glyph standing in for the tool's icon (keel uses an icon set; a
/// quiet monochrome character keeps the tile without shipping SVGs).
/// The glyph for a tool call (keel tool-chip.tsx `toolIcon`, Solar set).
...
mod tests {
    ...
    fn user_rows_project_file_mentions_into_chips() {
        let raw = "look at [composer.rs](keel-file:crates/ui/src/composer.rs) please";
    ...
    fn tool_diff_builds_real_hunks_with_context_and_numbers() {
        ...
        let created = keel_proto::ToolDiff {
            path: "/w/new.txt".into(),
            old_text: None,
            new_text: "only\n".into(),
        };
    ...
    fn call_block_carries_the_full_invocation() {
        ...
        let Some(ToolDetail::Output { lines, .. }) = call_block(&ToolCall::Mcp {
            server: "gh".into(),
            tool: "issues".into(),
            input: Some(serde_json::json!({"repo": "keel"})),
        }) else {
            panic!("expected an output block")
        };
        assert_eq!(lines[0].as_ref(), "gh · issues");
        assert!(lines.iter().any(|l| l.contains("\"repo\": \"keel\"")));
    ...
    #[test]
    fn decision_activity_uses_the_reported_backend_name() {
        let choosing = |backend| DecisionActivity {
            backend,
    ...
    #[test]
    fn decision_receipt_is_factual_backend_specific_and_not_stale_filtered() {
        let event = DecisionEvent::new(
            ...
            vec![
                keel_proto::DecisionCandidate::new("inspect", "Inspect repository"),
                keel_proto::DecisionCandidate::new("verify", "Run focused checks"),
            ],
        ...
        .with_selected_probability(0.7)
        .with_observed_outcome("repository mapped");
        let lines = decision_event_lines(&event);
        assert_eq!(lines[0], "Laya · Pre-step · revision 4");
        assert_eq!(lines[1], "Considered · inspect · Inspect repository");
        assert_eq!(lines[3], "Selected · inspect · validated");
        assert!(lines.iter().any(|line| line == "Confidence · 82%"));
        assert!(
            lines
                .iter()
                .any(|line| line == "Selected probability · 70%")
        );
        assert!(
            lines
                .iter()
                .any(|line| line == "Observed · repository mapped")
        );
        assert!(lines.iter().all(|line| !line.contains("reasoning")));

        // Durable receipts remain visible regardless of age; staleness only
        // applies to the transient `DecisionActivity` trailer.
        let entry = SessionMessageEntry {
            id: "decision-pre-step-1".into(),
        ...
        let rows = rows_for_entry(&entry, false, &mut parse);
        assert!(matches!(rows[0].kind, RowKind::Decision { .. }));
    }
