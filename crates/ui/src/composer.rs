...
use keel_doc::{MessagePart, MessageRole, SessionCommandPayload, SessionMessageEntry};
use keel_proto::{
    FileSearchMatch, HarnessId, RunRequest, SandboxLevel, SlashCommand, UserInputAnswer,
    UserInputQuestion,
};
use keel_rpc::{RpcError, methods};
...
/// Expanded-mode textarea vertical padding: `pt-4 pb-1` (keel composer.tsx
/// line 578) = 16 + 4.
...
/// 260)` (keel composer.tsx line 235). The 76px floor applies even when
...
/// children; composer/styles.tsx pickerChip) + `pb-2.5` (10) — keel
...
/// Staged-attachment strip metrics (keel attachment-ui.tsx AttachmentStrip:
...
fn pixel_send_symbol(color: gpui::Hsla, stop: bool) -> gpui::AnyElement {
    let pattern = if stop {
        [0b00000, 0b01110, 0b01110, 0b01110, 0b00000]
    } else {
        [0b00100, 0b01110, 0b11111, 0b00100, 0b00100]
    };
    div()
        .flex()
        .flex_col()
        .gap(px(1.0))
        .children(pattern.into_iter().map(|bits| {
            div()
                .flex()
                .gap(px(1.0))
                .children((0..5).map(move |column| {
                    div()
                        .size(px(2.0))
                        .when(bits & (1 << (4 - column)) != 0, |el| el.bg(color))
                }))
        }))
        .into_any_element()
}
...
const FILE_MENTION_SCHEME: &str = "keel-file:";
...
fn mention_error_message(err: &RpcError) -> SharedString {
    match err {
        RpcError::UnknownMethod(_) => {
            "The session's device runs an older Keel — update it to search its files".into()
        }
...
fn slash_error_message(err: &RpcError) -> SharedString {
    match err {
        RpcError::UnknownMethod(_) => {
            "The session's device runs an older Keel — update it to list commands".into()
        }
        RpcError::Transport(_) | RpcError::Closed => "The session's device is unreachable".into(),
...
pub struct Composer {
    ...
    /// `Window` — the `KEEL_ATTACH_PREVIEW` boot knob opens in `new`).
    ...
    current_key: String,
    sending: bool,
    /// Keystrokes extend the active pixel/clear window without resetting its
    /// phase until typing pauses; sends begin a fresh brief cycle.
    pixel_cycle_origin: Option<Instant>,
    pixel_last_activity: Option<Instant>,
...
impl Composer {
    ...
    pub fn new(state: Entity<AppState>, cx: &mut Context<Self>) -> Self {
        ...
        let input_events = cx.subscribe(&input, |this: &mut Self, _, event, cx| match event {
            ...
            ComposerInputEvent::Edited => {
                if this.wizard.is_none()
                    && !this.input.read(cx).is_empty()
                    && !this
                        .route_snap_until
                        .is_some_and(|until| Instant::now() < until)
                {
                    this.arm_pixel_motion(false);
                }
        ...
        let mut composer = Self {
            ...
            current_key,
            sending: false,
            pixel_cycle_origin: None,
            pixel_last_activity: None,
            failure: None,
            wizard: None,
        ...
        // a rig) — `KEEL_ATTACH=/path/a.png[,/path/b.png]`, and
        // `KEEL_ATTACH_PREVIEW=1` boots with the first one's lightbox open.
        if let Ok(spec) = std::env::var("KEEL_ATTACH") {
            let staged: Vec<StagedAttachment> = spec
                ...
                .filter_map(|path| {
                    match attachments::stage_file(std::path::Path::new(path.trim())) {
                        Ok(att) => Some(att),
                        Err(err) => {
                            tracing::warn!(%path, error = %err, "KEEL_ATTACH stage failed");
                            None
                        }
                    }
                })
                .collect();
            if std::env::var("KEEL_ATTACH_PREVIEW").is_ok_and(|v| v == "1")
    ...
    /// Capture-knob passthrough (`KEEL_OPEN_DIALOG=model`): open the
    ...
    fn arm_pixel_motion(&mut self, restart: bool) {
        let now = Instant::now();
        if restart
            || self.pixel_last_activity.is_none_or(|last| {
                now.duration_since(last) >= Duration::from_millis(motion::MESSAGE_PIXEL_ACTIVE_MS)
            })
        {
            self.pixel_cycle_origin = Some(now);
        }
        self.pixel_last_activity = Some(now);
    }
    ...
    fn send(&mut self, text: String, steer: bool, cx: &mut Context<Self>) {
        ...
        let decision_enabled = self
            .state
            .read(cx)
            .data_dir
            .as_deref()
            .and_then(|data_dir| onboarding::load_mode(data_dir).ok())
            .is_some_and(|mode| {
                matches!(
                    mode,
        ...
        if decision_enabled && self.pickers.read(cx).jev_auto_eligible(cx) {
            resolved
                .model_options
                .insert("jevAuto".into(), serde_json::Value::Bool(true));
        }
        ...
        let echo = SessionMessageEntry {
            id: message_id.clone(),
            role: keel_doc::MessageRole::User,
                    ...
                    match &plan {
                        ...
                        crate::pickers::CheckoutPlan::NewWorktree { base } => {
                            ...
                            if let (Some(repo_path), Some(base)) = (&space_path, base) {
                                ...
                                let worktree: keel_proto::Worktree = serde_json::from_value(value)
    ...
    /// The agent-asked-a-question panel (keel question-panel.tsx), rendered in
    ...
    fn render_wizard(&mut self, cx: &mut Context<Self>) -> gpui::AnyElement {
        ...
        let options = question.options.iter().enumerate().map(|(ix, label)| {
            ...
            // (typed answers win — keel question-panel.tsx `isSel`).
            ...
            div()
                ...
                // keel question-panel.tsx option rows: `transition-colors`.
                .bg(if picked {
    ...
    fn render_send_button(
        ...
        let theme = Theme::of(cx);
        // Keel composer-actions.tsx: a size-7 filled circle — up-arrow to
        ...
        match mode {
            SendButtonMode::Stop => div()
                ...
                .child(if pixelated {
                    pixel_send_symbol(theme.bg, true)
                } else {
                    div()
                        .size(px(11.0))
                        .rounded(px(3.0))
            ...
            SendButtonMode::Send | SendButtonMode::Steer => {
                ...
                div()
                    ...
                    .child(if pixelated {
                        pixel_send_symbol(theme.bg, false)
                    } else {
...
impl Render for Composer {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        // Centered composer column (keel `mx-auto w-full max-w-3xl`).
        let container = div()
            .w_full()
            .max_w(px(768.0))
            .mx_auto()
            ...
            .when_some(failure, |el, message| {
                // keel composer.tsx `Notice` (matches the transcript
        ...
        let steer_queues = mode == SendButtonMode::Steer
            && self.pickers.read(cx).resolved_steering_mode(cx)
                == Some(keel_proto::SteeringMode::TurnBoundary);
        ...
        // need the full-width actions row (keel composer-actions.tsx
        // `mustExpand = isNew || …`).
        ...
        let pixelated = match (self.pixel_cycle_origin, self.pixel_last_activity) {
            (Some(origin), Some(last))
                if !motion::reduced_motion(cx)
                    && now.duration_since(last)
                        < Duration::from_millis(motion::MESSAGE_PIXEL_ACTIVE_MS) =>
            {
                let view = cx.entity_id();
                motion::pulse_delta(&motion::MESSAGE_PIXEL, view, cx);
                motion::message_pixelated(now.duration_since(origin).as_millis() as u64)
            }
            _ => false,
        };
        let send_button = self.render_send_button(mode, pixelated, cx);
        ...
        let attach = div()
            ...
            .rounded_full()
            .cursor_pointer()
            // keel composer-actions.tsx attach: `transition-colors`.
        ...
        // The pill chrome (keel composer.tsx): `rounded-[26px] border
                    ...
                    div()
                        ...
                        .child(
                            div()
                                ...
                                // keel composer-actions.tsx): identical
...
mod tests {
    ...
    fn file_mentions_serialize_to_strict_local_markdown() {
        ...
        assert_eq!(
            raw,
            "[a file#\\[x\\].rs](keel-file:src/a%20file%23%5Bx%5D.rs)"
        );
        let links = file_mention_links(&raw);
        ...
        assert_eq!(folder, "[components](keel-file:src/components/)");
    ...
    fn sent_mention_display_leaves_plain_prompts_untouched() {
        ...
        assert_eq!(
            sent_mention_display("what is a keel-file: link?"),
            None,
            "scheme substring without a valid mention link"
        );
        assert_eq!(
            sent_mention_display("[a.rs](keel-file:../a.rs)"),
            None,
    ...
    #[test]
    fn auto_grow_math() {
        // The source heights (keel composer.tsx line 235 clamp, composer-
        ...
        // Caps at a 260px textarea box (keel max-h-[260px] / the JS clamp).
        assert_eq!(
