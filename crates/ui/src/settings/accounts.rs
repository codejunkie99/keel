...
//! account-shaped loading skeletons. This release presents locally detected
//! agents and account state on one device.
//!
...
/// Compact absolute reset moment (keel settings.agents.tsx `formatReset`):
...
/// in the empty-state copy, keel settings.agents.tsx `PROVIDERS`).
...
impl LoginFlow {
    /// Dialog title (keel: "Add Claude account" / "Add Codex account").
...
pub struct AccountsPage {
    ...
    installed_harnesses: Loadable<Vec<HarnessDescriptor>>,
...
impl AccountsPage {
    pub fn new(state: Entity<AppState>, cx: &mut Context<Self>) -> Self {
        ...
        let mut page = Self {
            ...
            installed_harnesses: Loadable::Idle,
            busy_account: None,
            login: None,
            error: None,
            code_input,
    ...
    // All account operations target the local runtime.
    ...
    fn load(&mut self, force_usage: bool, cx: &mut Context<Self>) {
        ...
        self.installed_harnesses = Loadable::Loading;
        let params = self.params(serde_json::json!({ "forceUsage": force_usage }));
        self.load_task = Some(cx.spawn(async move |this, cx| {
            let (harnesses, result) = tokio::join!(
                engine
                    .client()
            ...
            this.update(cx, |page, cx| {
                page.installed_harnesses = match harnesses {
            ...
            this.update(cx, |page, cx| {
                match result.and_then(|value| {
                    serde_json::from_value::<AgentLoginStart>(value)
                        .map_err(|e| keel_rpc::RpcError::Failed(e.to_string()))
                }) {
                    Ok(start) => {
    ...
    /// One usage window (keel settings.agents.tsx `UsageMeter`): label ·
    /// 5px rounded-full bar (indigo → amber ≥80% → red ≥95%) · "NN% used" ·
    /// quiet reset time.
    fn render_usage_meter(
        &self,
        window: &keel_proto::AgentUsageWindow,
        theme: &Theme,
        now: DateTime<Utc>,
    ) -> AnyElement {
    ...
    /// One account row (keel settings.agents.tsx `AccountRow`): initial
    ...
    fn render_account_row(
        ...
        // Actions only on INACTIVE accounts (keel `{!account.active && …}`):
            ...
            .child(
                div()
                    ...
                    .map(|el| {
                        ...
                        // (keel: `usage ? meters : "Usage unavailable"…`).
    ...
    fn render_login_dialog(
        ...
        let url_link =
            |id: &'static str, label: &'static str, url: &str, cx: &mut Context<Self>| {
                ...
                // "Reopen the …" text link (keel: `text-[12px]
    ...
    /// A ghost account row (keel settings.agents.tsx `SkeletonRow`): avatar,
    ...
    fn render_skeleton_row(
        ...
        use crate::motion;
        let delta = motion::pulse_delta(&motion::KEEL_PULSE, cx.entity_id(), cx);
...
impl Render for AccountsPage {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        // Brand mark inside a 24px centered box (keel: `grid size-6
        ...
        // One section per provider (keel settings.agents.tsx `ProviderSection`):
            ...
            Loadable::Ready(snapshot) => {
                ...
                PROVIDERS
                    ...
                    .map(|(harness, name, cli)| {
                        ...
                        // EVERY warning renders its own strip (keel maps them).
        ...
        let local_tools: Vec<AnyElement> = match &self.installed_harnesses {
                ...
                .map(|harness| {
                    let signed_in = self.snapshot.ready().is_some_and(|snapshot| {
                        snapshot
                            .accounts
                            .iter()
                            .any(|account| account.harness == harness.id && account.active)
                    });
                    let status = if !harness.installed {
                        "Not installed"
                    } else if signed_in {
                        "Installed · signed in locally"
                    } else if matches!(harness.id, HarnessId::ClaudeCode | HarnessId::Codex) {
                        "Installed · sign in with this CLI"
                    } else {
                        "Installed · sign-in status managed by this CLI"
                    };
                    div()
                        .py(px(8.0))
                        .flex()
                        ...
                        .child(
                            div()
                                .text_size(px(12.5))
                                .child(SharedString::from(harness.name.clone())),
                        )
                        .child(
                            div()
                                .text_size(px(11.5))
                                .text_color(if harness.installed {
                                    theme.success
                                } else {
                                    theme.text_muted
                                })
            ...
            Loadable::Error(error) => {
                vec![widgets::error_strip(&theme, error.clone()).into_any_element()]
            }
            Loadable::Idle | Loadable::Loading => vec![
                div()
                    .text_size(px(12.0))
                    .text_color(theme.text_muted)
                    .child(SharedString::from("Checking installed coding tools…"))
                    .into_any_element(),
            ],
        };
        div()
            ...
            .child(
                widgets::page_column()
                    .child(
                        div()
                            ...
                            .child(
                                ...
                                // dimmed while a refresh is in flight (keel
                                // `disabled:opacity-50`).
                                widgets::ghost_action(&theme)
                                    .id("accounts-refresh")
                                    .flex_none()
                    ...
                    .child(widgets::page_subtitle(
                        &theme,
                        "This device uses installed coding CLIs and their own local sign-ins. \
                         Keel reads local account state for Claude Code and Codex.",
                    ))
                    .child(
                        div()
                            .mt(px(18.0))
                            .text_size(px(15.0))
                            .font_weight(gpui::FontWeight::SEMIBOLD)
                            .child(SharedString::from("Installed local tools")),
                    )
                    .child(
                        widgets::section_card(&theme)
                            .mt(px(8.0))
                            .p(px(14.0))
                            .children(local_tools),
                    )
                    ...
                    .children(sections)
                    // Footer note (keel: `mt-6 text-[12px] leading-relaxed
...
mod tests {
    ...
    #[test]
    fn usage_thresholds_match_keel() {
