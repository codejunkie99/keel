//! The app shell (keel `__root.tsx`): sidebar column + main panel + optional
//! right "Changes" pane, plus the local engine boot state and connection gate.
//!
//! Layout is keel's: collapsible drag-resizable sidebar (208–400px, default
...
use gpui::{
    ...
    Keystroke, MouseButton, MouseDownEvent, MouseUpEvent, Pixels, Point, Render, ScrollHandle,
    SharedString, Subscription, Task, Window, WindowControlArea, actions, div, prelude::*, px,
};

use gpui_tokio::Tokio;
use keel_engine::InstanceLock;
...
use crate::loaders;
use crate::motion::{self, AnimationExt as _, KEEL_PULSE, MotionSpec, RESIZE, TAB_SLIDE};
use crate::onboarding;
use crate::onboarding::LayaInstallState;
...
/// left edge (keel window-controls.tsx: `left: fullscreen ? 12 : 88`). The
...
impl SettingsSection {
    ...
    /// Sidebar + header label (keel settings-sidebar.tsx SECTIONS / __root.tsx
...
/// Per-chat panel open flags (keel parity: `sessionPanels` — the terminal and
...
/// One route-history entry (keel parity: the renderer's TanStack memory
...
/// (keel window-controls.tsx semantics): every route change pushes an entry;
...
impl NavHistory {
    ...
    /// equivalent of a `replace: true` navigation (keel's boot redirect from
    ...
    /// "can go forward" (keel window-controls.tsx).
...
fn account_menu_action(scope: Option<WorkspaceScope>, flow: SyncFlow) -> Option<AccountMenuAction> {
    match scope {
        Some(WorkspaceScope::Local) => None,
...
pub struct Shell {
    ...
    sound_prev: std::collections::HashMap<String, keel_proto::SessionStatus>,
    ...
    install: keel_update::InstallKind,
    ...
    data_dir: PathBuf,
    settings: UiSettings,
    onboarding_error: Option<SharedString>,
    laya_available: bool,
    laya_downloadable: bool,
    laya_install: LayaInstallState,
    laya_status: SharedString,
    laya_install_task: Option<Task<()>>,
    ...
    /// Dev/testing knobs (`KEEL_OPEN_DIALOG`, `KEEL_FORCE_GATE`) — see
    /// [`Shell::new`].
    ...
    intro: IntroPhase,
    intro_task: Option<Task<()>>,
    intro_started: Option<std::time::Instant>,
    /// The modal intro/setup surface owns keyboard focus until it closes.
    overlay_focus: FocusHandle,
    /// Active keyboard action (intro: 0–1; setup: 0–4).
    overlay_action: usize,
    onboarding_scroll: ScrollHandle,
...
impl Shell {
    pub fn new(state: Entity<AppState>, boot: EngineBootConfig, cx: &mut Context<Self>) -> Self {
        ...
        let laya = onboarding::laya_status(&data_dir);
        // Bind the customizable shortcuts from the persisted keymap.
        apply_keymap(cx, &settings.keymap);
        // Dev/testing knob: `KEEL_OPEN_ROUTE=settings[/<section>]` boots
        // straight into a settings section — these pages have no deep link and
        // synthetic input can't reach them on headless compositors.
        let route = match std::env::var("KEEL_OPEN_ROUTE").ok().as_deref() {
            Some("settings") | Some("settings/devices") => {
                Route::Settings(SettingsSection::Devices)
            }
            Some("settings/onboarding") => Route::Settings(SettingsSection::Onboarding),
        ...
        // More capture knobs of the same kind: `KEEL_OPEN_DIALOG=rename|delete`
        // opens that dialog for the first chat once chats land; `=model` pops
        // the combined harness/model menu once the shell is Ready;
        // `KEEL_FORCE_GATE=signin|org|failed` renders that gate regardless of
        // real auth state (display-only — for styling passes).
        let debug_dialog = std::env::var("KEEL_OPEN_DIALOG").ok();
        let debug_gate = match std::env::var("KEEL_FORCE_GATE").ok().as_deref() {
            Some("signin") => Some(GatePhase::SignIn),
            Some("org") => Some(GatePhase::OrgGate),
            Some("failed") => Some(GatePhase::Failed(
                "Could not reach the keel engine on port 27901".into(),
            )),
            _ => None,
        };
        ...
        Self {
            ...
            install: keel_update::detect_install(),
            org: None,
            ...
            laya_available: laya.installed,
            laya_downloadable: laya.downloadable,
            laya_install: LayaInstallState::Idle,
            laya_status: laya.message.into(),
            laya_install_task: None,
            ...
            intro: IntroPhase::AwaitEngine,
            intro_task: None,
            intro_started: None,
            overlay_focus: cx.focus_handle(),
            overlay_action: 0,
            onboarding_scroll: ScrollHandle::new(),
            save_task: None,
            focus_sub: None,
            _ticker: ticker,
    ...
    fn start_intro(&mut self, cx: &mut Context<Self>) {
        if self.intro != IntroPhase::AwaitEngine {
            return;
        }
        if self.reduced_motion {
            self.intro = IntroPhase::ReadyForAction;
            return;
        }
        self.intro = IntroPhase::Animating;
        self.intro_started = Some(std::time::Instant::now());
        self.intro_task = Some(cx.spawn(async move |this, cx| {
            cx.background_executor()
                .timer(Duration::from_millis(KEEL_PULSE.duration_ms))
                .await;
            this.update(cx, |shell, cx| {
                if shell.intro == IntroPhase::Animating {
                    shell.intro = IntroPhase::ReadyForAction;
                    cx.notify();
                }
            })
            .ok();
    ...
    fn on_state_changed(&mut self, state: &Entity<AppState>, cx: &mut Context<Self>) {
        let next_sync_flow = {
            ...
            sync_flow_after_auth(
                ...
                state.auth.as_ref(),
                self.boot.workos_client_id.is_some(),
            )
        };
        ...
        {
            let now = Utc::now();
            type Ping = (String, keel_proto::SessionStatus, bool, Option<String>);
            let sessions: Vec<Ping> = {
                ...
                state
                    ...
                    .iter()
                    .map(|s| {
                        use keel_proto::view::Indicator;
                        let status = match keel_proto::view::effective_indicator(Some(s), now) {
                            Indicator::Working => keel_proto::SessionStatus::Working,
                            Indicator::AwaitingInput => keel_proto::SessionStatus::AwaitingInput,
                            Indicator::Errored => keel_proto::SessionStatus::Errored,
                            Indicator::None => keel_proto::SessionStatus::Idle,
                        };
            ...
            // Keel window being key), so a ping for a *background chat* in a
            // focused app still stays a chime — you're already looking at
            // Keel; the sidebar dot carries the rest.
        ...
        if selected != self.active_chat {
            ...
            // keel's `/` route redirected into the last-used chat, leaving no
    ...
    fn add_commit_diff_surface(
        &mut self,
        commit: keel_proto::GitHistoryCommit,
        cx: &mut Context<Self>,
    ) {
    ...
    /// The flag is per chat (keel `sessionPanels`).
    fn toggle_terminal(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        if open {
            ...
            // to the prompt, no click needed (keel terminal-panel.tsx: the
            ...
            // press closes even while the terminal is focused, as in keel's
    ...
    fn on_right_pane_drag(
        ...
        // keel caps the pane at 52% of the window on top of the absolute range.
    ...
    fn open_settings(&mut self, section: SettingsSection, cx: &mut Context<Self>) {
        ...
        // Keep an active download observer alive while navigating away and back.
        if section == SettingsSection::Harnesses
            && !self
                .harnesses_page
                .as_ref()
                .is_some_and(|page| page.read(cx).is_installing_laya())
        {
            self.harnesses_page = None;
            self.harnesses_sub = None;
        }
        if section == SettingsSection::Harnesses {
    ...
    fn settings_outlet(&mut self, section: SettingsSection, cx: &mut Context<Self>) -> AnyElement {
        match section {
            ...
            SettingsSection::Onboarding => self.render_settings_onboarding(cx),
            SettingsSection::Harnesses => {
                if self.harnesses_page.is_none() {
                    let state = self.state.clone();
                    let data_dir = self.data_dir.clone();
                    let install = self.laya_install;
                    let page = cx.new(|cx| HarnessesPage::new(state, data_dir, install, cx));
                    self.harnesses_sub = Some(cx.observe(&page, |shell: &mut Self, page, cx| {
                        let Some((install, status, error)) =
                            page.read(cx).owned_laya_install_status()
                        else {
                            return;
                        };
                        shell.laya_install = install;
                        shell.laya_available = status.installed;
                        shell.laya_downloadable = status.downloadable;
                        shell.laya_status = status.message.into();
                        shell.onboarding_error = error.map(Into::into);
                        cx.notify();
                    }));
                ...
                match result {
                    ...
                    Err(err) => {
                        shell.runtime_change_error = Some(format!(
                            "Could not stop the remote engine: {err}. Run `keel daemon stop`, then quit and reopen Keel."
                        ).into());
                        cx.notify();
                    }
                }
    ...
    /// reduced motion: exactly `target`. Honors `KEEL_MOTION_SCALE`.
    ...
    /// of keel __root.tsx `transition-[padding-left] duration-200 ease-out` +
    ...
    /// by every route transition (keel remounts the keyed header variants —
    ...
    /// pattern (keel's `.drag` region): mark it a [`WindowControlArea::Drag`]
    ...
    /// keel window-controls.tsx): rendered once, in a paint-only overlay layer
    ...
    /// Native Windows caption controls integrated into Keel's unified
    ...
    /// Settings-mode sidebar (keel settings-sidebar.tsx): window-control
    ...
    fn render_settings_nav(
        ...
        let section_icon = |item: SettingsSection| match item {
            ...
            SettingsSection::Onboarding => icons::KEEL_LOGO,
        ...
        div()
            ...
            // Back pinned to the bottom (keel settings-sidebar.tsx).
            .child(
                div().px(px(Theme::SPACE_SM)).pb(px(12.0)).child(
                    div()
                        ...
                        .child(
                            // AltArrowLeft chevron (keel settings-sidebar.tsx),
    ...
    /// One session row (keel session-row.tsx): status rail on the left
    ...
    fn render_chat_row(
        ...
        harness: Option<keel_proto::HarnessId>,
        status: keel_proto::ChatIndicator,
        selected: bool,
        archived: bool,
        theme: &Theme,
        cx: &mut Context<Self>,
        ...
        let status_label: Option<&'static str> = match status {
            keel_proto::ChatIndicator::Working => Some("Working"),
            keel_proto::ChatIndicator::AwaitingInput => Some("Input"),
            keel_proto::ChatIndicator::Errored => Some("Failed"),
            keel_proto::ChatIndicator::Completed => Some("Done"),
            keel_proto::ChatIndicator::Idle => None,
        };
        let corner_body: AnyElement = if corner_hovered {
            ...
            match status_label {
                Some(label) => {
                    ...
                    let glyph: AnyElement = if status == keel_proto::ChatIndicator::Completed {
                        icon(icons::CHECK)
                            .size(px(11.0))
                            .flex_none()
        ...
        // Hover fades over transition-colors (keel session-row.tsx) — both
        ...
        div()
            ...
            .child(
                div()
                    ...
                    .when(status == keel_proto::ChatIndicator::Working, |el| {
                        el.child(div().flex_1())
    ...
    /// (`keel update`); click dismisses it for that version.
    fn render_update_strip(&mut self, theme: &Theme, cx: &mut Context<Self>) -> Option<AnyElement> {
        ...
        let mac_app = matches!(self.install, keel_update::InstallKind::MacApp { .. });
        ...
        let (label, clickable): (SharedString, bool) = if mac_app {
            ...
            (
                format!("Update available — v{latest} · run `keel update`").into(),
                true,
            )
        };
    ...
    fn on_update_strip_click(&mut self, cx: &mut Context<Self>) {
        if !matches!(self.install, keel_update::InstallKind::MacApp { .. }) {
    ...
    /// Fetch the manifest and stage the new Keel desktop bundle under the data dir
    ...
    fn begin_update_download(&mut self, cx: &mut Context<Self>) {
        ...
        let download = Tokio::spawn(cx, async move {
            let manifest = keel_update::fetch_latest(&edge_url).await?;
            keel_update::stage_mac_app(&edge_url, &manifest, &data_dir).await
        });
    ...
    fn apply_staged_update(&mut self, staged: PathBuf, cx: &mut Context<Self>) {
        let keel_update::InstallKind::MacApp { bundle } = self.install.clone() else {
            return;
        };
        match keel_update::apply_mac_app(&staged, &bundle) {
            Ok(()) => {
                keel_update::relaunch_app_after_exit(&bundle);
                cx.quit();
            }
            Err(err) => {
    ...
    fn render_user_menu(
        ...
        let mut trigger = div()
            ...
            .child(
                // Avatar: white circle, initial in near-black (keel user-menu.tsx).
                div()
                    .size(px(28.0))
                    .flex_none()
                    .rounded_full()
        ...
        let card = match self.sync_flow {
            SyncFlow::Enabling => popover::dialog_card(&theme)
                ...
                .child(
                    div().mt(px(6.0)).child(popover::dialog_body(
                        &theme,
                        "Finish signing in in your browser. Keel will keep using this local workspace until you quit and reopen.",
                    )),
                )
                .child(
                    div()
            ...
            SyncFlow::SwitchOffer { notice_open: true } => {
                ...
                let body: SharedString = match (&signed_in_email, &work_phrase) {
                    ...
                    (Some(email), None) => format!(
                        "You're signed in as {email}. Keel can switch to your synced workspace now."
                    )
                    .into(),
                    ...
                    .into(),
                    (None, None) => "Keel can switch to your synced workspace now.".into(),
                };
                let mut actions = div()
                    .mt(px(16.0))
                    .flex()
            ...
            SyncFlow::RestartPending { notice_open: true } => popover::dialog_card(&theme)
                ...
                .child(
                    div().mt(px(6.0)).child(popover::dialog_body(
                        &theme,
                        if remote_engine {
                            "Keel is using a background daemon. Stop it and quit Keel, then reopen to start the synced workspace. Existing local sessions stay on this device and will not be uploaded."
                        } else {
                            "Quit and reopen Keel to start the synced workspace. Existing local sessions stay on this device and will not be uploaded."
                        },
                    )),
                )
                .when_some(self.runtime_change_error.clone(), |card, error| {
            ...
            SyncFlow::SignOutConfirm => popover::dialog_card(&theme)
                ...
                .child(
                    div().mt(px(6.0)).child(popover::dialog_body(
                        &theme,
                        "Keel will remove your credentials, close the synced workspace, and continue in local mode.",
                    )),
                )
                .child(
                    div()
    ...
    fn render_main(&mut self, cx: &mut Context<Self>) -> AnyElement {
        ...
        let outlet: AnyElement = if has_selection {
            ...
            div()
                ...
                .child(motion::fade_in(
                    ...
                    div()
                        ...
                        .items_center()
                        .child(
                            icon(icons::KEEL_LOGO)
                                .size(px(48.0))
                                .text_color(gpui::rgb(0x59e6ce).opacity(0.09)),
                        )
                        .child(
                            div()
                                .mt(px(24.0))
            ...
            // New-chat canvas (keel index.tsx): the keel mark over the
                ...
                .child(motion::fade_in(
                    ...
                    div()
                        ...
                        .child(
                            icon(icons::KEEL_LOGO)
                                .size(px(48.0))
                                // Keep the mark muted but visible on glass.
                                .text_color(gpui::rgb(0x59e6ce).opacity(0.2)),
                        )
    ...
    fn render_status_strip(&mut self, cx: &mut Context<Self>) -> AnyElement {
        ...
        // inner gutter (keel's `mx-auto h-6 max-w-3xl px-2`).
        let strip = div()
        ...
        let card = div()
            ...
            .child(
                div()
                    ...
                    .text_color(theme.text_muted)
                    .child(SharedString::from(
                        "Keel removed your credentials but could not finish closing the previous synced workspace. Retry before continuing in local mode.",
                    )),
            )
            .when_some(self.runtime_change_error.clone(), |card, error| {
                card.child(
    ...
    fn select_decision_mode(&mut self, mode: onboarding::DecisionMode, cx: &mut Context<Self>) {
        if mode == onboarding::DecisionMode::Laya && !self.laya_available {
            self.onboarding_error = Some(self.laya_status.clone());
            cx.notify();
            return;
        }
        if mode == onboarding::DecisionMode::Jev
            && keel_engine::decision_mode::protected_typesafe_key_path().is_none()
        {
            self.onboarding_error =
                Some("Jev needs an existing protected TypeSafe credential on this Mac.".into());
            cx.notify();
            return;
        }
        self.onboarding_error = onboarding::save_mode(&self.data_dir, mode)
            .err()
            .map(|err| format!("Could not save decision mode: {err}").into());
        if self.onboarding_error.is_none() {
            self.sync_decision_mode_to_settings(cx);
        }
        cx.notify();
    }

    fn sync_decision_mode_to_settings(&self, cx: &mut Context<Self>) {
        if let Some(page) = &self.harnesses_page {
            page.update(cx, |page, cx| page.refresh_decision_mode(cx));
        }
    }

    fn sync_laya_to_settings(&self, refresh: bool, cx: &mut Context<Self>) {
        if let Some(page) = &self.harnesses_page {
            page.update(cx, |page, cx| {
                page.sync_laya_from_shell(self.laya_install, refresh, cx)
            });
        }
    }

    fn install_laya(&mut self, cx: &mut Context<Self>) {
        if self.laya_available || !self.laya_downloadable || self.laya_install.is_running() {
            return;
        }
        if let Err(err) = onboarding::save_mode(&self.data_dir, onboarding::DecisionMode::Laya) {
            self.onboarding_error = Some(format!("Could not select Laya: {err}").into());
            cx.notify();
            return;
        }
        self.laya_install = LayaInstallState::Starting;
        self.onboarding_error = None;
        self.sync_laya_to_settings(true, cx);
        let data_dir = self.data_dir.clone();
        let install_dir = data_dir.clone();
        let (progress_tx, mut progress_rx) = tokio::sync::mpsc::unbounded_channel();
        let install = Tokio::spawn(cx, async move {
            keel_engine::decision_mode::install_laya_model_with_progress(&install_dir, progress_tx)
                .await
        });
        self.laya_install_task = Some(cx.spawn(async move |this, cx| {
            while let Some(progress) = progress_rx.recv().await {
                this.update(cx, |shell, cx| {
                    shell.laya_install = progress.into();
                    shell.sync_laya_to_settings(false, cx);
                    cx.notify();
                })
                .ok();
            }
            let outcome = install.await;
            let status = onboarding::laya_status(&data_dir);
            this.update(cx, |shell, cx| {
                shell.laya_install = LayaInstallState::Idle;
                shell.laya_available = status.installed;
                shell.laya_downloadable = status.downloadable;
                shell.laya_status = status.message.into();
                shell.onboarding_error = match outcome {
                    Ok(Ok(_)) if shell.laya_available => None,
                    Ok(Ok(_)) => {
                        Some("Laya finished downloading but could not be activated.".into())
                    }
                    Ok(Err(error)) if error.to_string().contains("already running") => {
                        Some("Laya is already downloading elsewhere.".into())
                    }
                    Ok(Err(error)) => Some(format!("Laya download failed: {error}").into()),
                    Err(error) => Some(format!("Laya download failed: {error}").into()),
                };
                shell.sync_laya_to_settings(true, cx);
                cx.notify();
            })
            .ok();
        }));
    ...
    fn finish_decision_onboarding(
        &mut self,
        mode: onboarding::DecisionMode,
        connect_accounts: bool,
        cx: &mut Context<Self>,
    ) {
        if mode == onboarding::DecisionMode::Jev
            && keel_engine::decision_mode::protected_typesafe_key_path().is_none()
        ...
        if let Err(err) = onboarding::save_mode(&self.data_dir, mode) {
            self.onboarding_error = Some(format!("Could not save decision mode: {err}").into());
            cx.notify();
            return;
        }
        let mut next = self.settings.clone();
        next.jev_onboarding_complete = true;
        if let Err(err) = next.save(&self.data_dir) {
            self.onboarding_error = Some(format!("Could not save setup: {err}").into());
            cx.notify();
            return;
        }
        self.settings = next;
        ...
        } else if self.route == Route::Settings(SettingsSection::Onboarding) {
            self.close_settings(cx);
        }
        cx.notify();
    }

    fn skip_decision_onboarding(&mut self, cx: &mut Context<Self>) {
        self.finish_decision_onboarding(onboarding::DecisionMode::Normal, false, cx);
    }

    fn onboarding_first_action(&self) -> usize {
        if self.laya_available || self.laya_downloadable {
            0
        } else if keel_engine::decision_mode::protected_typesafe_key_path().is_some() {
            1
        } else {
            2
        }
    ...
    fn dismiss_intro(&mut self, new_session: bool, cx: &mut Context<Self>) {
        self.intro = IntroPhase::Dismissed;
        self.overlay_action = self.onboarding_first_action();
        if new_session {
    ...
    fn on_overlay_key_down(&mut self, event: &gpui::KeyDownEvent, cx: &mut Context<Self>) {
        let key = event.keystroke.key.as_str();
        let intro = self.intro != IntroPhase::Dismissed
            && self.route != Route::Settings(SettingsSection::Onboarding);
        let jev_available = keel_engine::decision_mode::protected_typesafe_key_path().is_some();
        let available = if intro {
            [
                true,
                self.settings.jev_onboarding_complete,
                false,
                false,
                false,
            ]
            ...
            [
                self.laya_available || (self.laya_downloadable && !self.laya_install.is_running()),
                jev_available,
                true,
                true,
                true,
        ...
        if key == "tab" {
            ...
            for _ in 0..count {
                self.overlay_action = (self.overlay_action + step) % count;
                if available[self.overlay_action] {
                    break;
                }
            }
            if !intro {
                match self.overlay_action {
                    0 => self.onboarding_scroll.scroll_to_item(3),
                    1 => self.onboarding_scroll.scroll_to_item(4),
                    _ => self.onboarding_scroll.scroll_to_bottom(),
                }
            }
            cx.stop_propagation();
            cx.notify();
        ...
        cx.stop_propagation();
        if intro {
            if self.intro == IntroPhase::ReadyForAction {
                self.dismiss_intro(self.overlay_action == 1, cx);
            }
            return;
        }
        let (saved_mode, _) = match onboarding::load_mode(&self.data_dir) {
            Ok(mode) => (mode, None),
            Err(err) => (onboarding::DecisionMode::Normal, Some(err)),
        };
        match self.overlay_action {
            0 if self.laya_available => {
                self.select_decision_mode(onboarding::DecisionMode::Laya, cx)
            }
            0 if self.laya_downloadable && !self.laya_install.is_running() => self.install_laya(cx),
            1 if jev_available => self.select_decision_mode(onboarding::DecisionMode::Jev, cx),
            2 => self.skip_decision_onboarding(cx),
            3 => self.finish_decision_onboarding(saved_mode, true, cx),
            4 => self.finish_decision_onboarding(saved_mode, false, cx),
            _ => {}
        }
    }

    fn render_intro(&mut self, cx: &mut Context<Self>) -> AnyElement {
        let theme = Theme::of(cx).clone();
        let animating = self.intro == IntroPhase::Animating;
        let mark = loaders::ascii_keel_mark(
            &theme,
            9.0,
            animating,
            self.intro_started,
        ...
        let actions: AnyElement = if self.intro == IntroPhase::ReadyForAction {
            let buttons =
                div()
                    .flex()
                    .flex_row()
                    .items_center()
                    .gap(px(8.0))
                    .child(overlay_action_ring(
                        &theme,
                        self.overlay_action == 0,
                        popover::btn_primary(&theme, "Continue")
                            .id("intro-continue")
                            .on_click(cx.listener(|this, _, _, cx| this.dismiss_intro(false, cx))),
                    ));
            let buttons = if self.settings.jev_onboarding_complete {
                buttons.child(overlay_action_ring(
                    &theme,
                    self.overlay_action == 1,
                    popover::btn_ghost(&theme, "Start a new session", "intro-new-session-hover")
                        .id("intro-new-session")
                        .on_click(cx.listener(|this, _, _, cx| this.dismiss_intro(true, cx))),
                ))
            } else {
                buttons
            };
            motion::fade_in("intro-actions", buttons).into_any_element()
        } else {
            div().h(px(42.0)).into_any_element()
        };
        div()
            .absolute()
            .inset_0()
            ...
            .on_key_down(
                cx.listener(|this, ev: &gpui::KeyDownEvent, _, cx| {
                    this.on_overlay_key_down(ev, cx)
                }),
            )
            .bg(theme.bg)
            .child(grid_backdrop(&theme))
                        ...
                        div()
                            ...
                            .child(
                                div()
                                    .text_size(px(13.0))
                                    .text_color(theme.text_muted)
                                    .child("Your tools, one local workspace."),
                            )
                            .child(div().mt(px(20.0)).child(actions))
                            .when(self.intro == IntroPhase::ReadyForAction, |content| {
                                content.child(
                                    div()
                                        .text_size(px(11.0))
                                        .text_color(theme.text_muted)
                                        .child("Tab to choose · Enter or Space to continue"),
                                )
                            }),
                    ),
            )
    ...
    fn decision_onboarding_card(
        &mut self,
        in_settings: bool,
        cx: &mut Context<Self>,
    ) -> AnyElement {
        let theme = Theme::of(cx).clone();
        let (saved_mode, mode_error) = match onboarding::load_mode(&self.data_dir) {
            Ok(mode) => (mode, None),
            Err(err) => (
                onboarding::DecisionMode::Normal,
                Some(format!("Could not read decision mode: {err}").into()),
            ),
        };
        let error = self.onboarding_error.clone().or(mode_error);
        let border = theme.border;
        let jev_available = keel_engine::decision_mode::protected_typesafe_key_path().is_some();
        let active_mode =
            onboarding::available_mode(saved_mode, self.laya_available, jev_available);
        let install_label = self.laya_install.label();
        let install_fraction = self
            .laya_install
            .fraction()
            .or_else(|| (self.laya_install == LayaInstallState::Verifying).then_some(1.0));
        let local = div()
            .w_full()
            .p(px(16.0))
            .rounded(px(10.0))
            ...
            .child(
                div()
                    .text_size(px(14.0))
                    .font_weight(gpui::FontWeight::SEMIBOLD)
                    .child(SharedString::from("Laya · local default")),
            )
            .child(
                div()
                    .text_size(px(12.0))
                    .line_height(px(18.0))
                    .text_color(theme.text_muted)
                    .child(SharedString::from("The local Core ML model selects eligible model routes and suggests bounded steps. Your installed coding tools keep their own coding loops and local sign-ins.")),
            )
            .child(
                div()
                    .text_size(px(11.5))
                    .text_color(if self.laya_available { theme.success } else { theme.warning })
                    .child(self.laya_status.clone()),
            )
            .when(saved_mode == onboarding::DecisionMode::Laya, |card| {
                card.child(
                    div()
                        .text_size(px(11.5))
                        .text_color(theme.text_muted)
            ...
            .child(
                div()
                    ...
                    .rounded(px(7.0))
                    .border_1()
                    .border_color(if self.overlay_action == 0 { theme.accent } else { border })
                    .text_size(px(12.5))
                    .when(self.laya_available, |button| button.cursor_pointer().on_click(cx.listener(|this, _, _, cx| {
                        this.overlay_action = 0;
                        this.select_decision_mode(onboarding::DecisionMode::Laya, cx)
                    })))
                    .when(self.laya_downloadable && !self.laya_install.is_running(), |button| button.cursor_pointer().on_click(cx.listener(|this, _, _, cx| {
                        this.overlay_action = 0;
                        this.install_laya(cx)
                    })))
                    .when(!self.laya_available && (!self.laya_downloadable || self.laya_install.is_running()), |button| button.opacity(0.5))
                    .child(SharedString::from(if self.laya_install.is_running() {
                        "Downloading Laya…"
                    } else if self.laya_downloadable {
                        "Download Laya (≈680 MB)"
                    } else if !self.laya_available {
                        "Laya unavailable"
                    } else if active_mode == onboarding::DecisionMode::Laya {
                        "Laya selected"
                    } else {
                        "Choose Laya"
                    })),
            )
            .when(self.laya_install.is_running(), |card| {
                card.child(
                    div()
                        .w_full()
                        .flex()
                        ...
                        .when_some(install_fraction, |progress, fraction| {
                            progress.child(
                                div()
                                    .w_full()
                                    .h(px(5.0))
        ...
        let jev = div()
            ...
            .child(
                div()
                    ...
                    .line_height(px(18.0))
                    .text_color(theme.text_muted)
                    .child("Uses an existing protected local credential for bounded decisions. Keel has no key-entry screen; selected decision context is sent directly to TypeSafe."),
            )
            .child(
                div()
                    .text_size(px(11.5))
                    .text_color(if jev_available { theme.success } else { theme.warning })
                    .child(if jev_available { "Protected credential found" } else { "No protected TypeSafe credential found" }),
            )
            .child(
                div()
                    .id("jev-select")
                    ...
                    .rounded(px(7.0))
                    .border_1()
                    .border_color(if self.overlay_action == 1 { theme.accent } else { border })
                    .text_size(px(12.5))
                    .when(jev_available, |button| button.cursor_pointer().on_click(cx.listener(|this, _, _, cx| {
                        this.overlay_action = 1;
                        this.select_decision_mode(onboarding::DecisionMode::Jev, cx)
                    })))
                    .when(!jev_available, |button| button.opacity(0.5))
                    .child(if !jev_available { "Jev unavailable" } else if active_mode == onboarding::DecisionMode::Jev { "Jev selected" } else { "Choose Jev" }),
            );
        let card = div()
            .id("decision-onboarding-card")
            .w(px(600.0))
            .max_h(px(540.0))
            .overflow_y_scroll()
            .track_scroll(&self.onboarding_scroll)
            .p(px(28.0))
            .rounded(px(14.0))
            .border_1()
            .border_color(border)
            ...
            .flex_col()
            .items_start()
            .child(icon(icons::KEEL_LOGO).size(px(36.0)).text_color(gpui::rgb(0x59e6ce)))
            .child(
                div()
                    .mt(px(14.0))
                    .text_size(px(23.0))
                    .font_weight(gpui::FontWeight::SEMIBOLD)
                    .child(if in_settings { "Onboarding" } else { "Welcome to Keel" }),
            )
            .child(
                div()
                    .mt(px(6.0))
            ...
            .child(
                div()
                    ...
                    .line_height(px(18.0))
                    .text_color(theme.text_muted)
                    .child(SharedString::from("Review local tools in Accounts. Keel detects installed CLIs and their available account state; each tool uses its own local sign-in.")),
            )
            .child(
                div()
                    .mt(px(8.0))
                    .text_size(px(11.0))
                    .text_color(theme.text_muted)
                    .child("Tab to choose · Enter or Space to activate"),
            )
            .when_some(error, |card, error| card.child(
                div().mt(px(12.0)).text_size(px(12.0)).text_color(theme.danger).child(error),
            ))
            .child(
                div()
                    .w_full()
                    ...
                    .child(
                        overlay_action_ring(
                            &theme,
                            self.overlay_action == 2,
                            popover::btn_ghost(
                                &theme,
                                "Use normal harness",
                                "decisions-skip-hover",
                            )
                            .id("decisions-skip")
                            .on_click(
                                cx.listener(|this, _, _, cx| this.skip_decision_onboarding(cx)),
                            ),
                        ),
                    )
                    .child(
                        div()
                            ...
                            .child(overlay_action_ring(
                                &theme,
                                self.overlay_action == 3,
                                popover::btn_ghost(
                                    &theme,
                                    "Review local tools",
                                    "decisions-review-hover",
                                )
                                .id("decisions-connect-accounts")
                                .on_click(cx.listener(move |this, _, _, cx| {
                                    this.finish_decision_onboarding(saved_mode, true, cx)
                                })),
                            ))
                            .child(overlay_action_ring(
                                &theme,
                                self.overlay_action == 4,
                                popover::btn_primary(
                                    &theme,
                                    if in_settings {
                                        "Done"
                                    } else {
                                        match (saved_mode, active_mode) {
                                            (onboarding::DecisionMode::Laya, onboarding::DecisionMode::Normal) => "Open Keel · Laya pending",
                                            (_, onboarding::DecisionMode::Normal) => "Open Keel",
                                            (_, onboarding::DecisionMode::Laya) => "Open Keel with Laya",
                                            (_, onboarding::DecisionMode::Jev) => "Open Keel with Jev",
                                        }
                                    },
                                )
                                .id("decisions-finish")
                                .on_click(cx.listener(move |this, _, _, cx| {
                                    this.finish_decision_onboarding(saved_mode, false, cx)
                                })),
                            )),
                    ),
            );
    ...
    fn render_local_onboarding(&mut self, cx: &mut Context<Self>) -> AnyElement {
        let theme = Theme::of(cx).clone();
        let card = self.decision_onboarding_card(false, cx);
        div()
            .absolute()
            .inset_0()
            .track_focus(&self.overlay_focus)
    ...
    fn render_settings_onboarding(&mut self, cx: &mut Context<Self>) -> AnyElement {
        let theme = Theme::of(cx).clone();
        let card = self.decision_onboarding_card(true, cx);
        div()
    ...
    fn render_gate_card(&mut self, phase: &GatePhase, cx: &mut Context<Self>) -> AnyElement {
        ...
        let content: AnyElement = match phase {
            // Backend unreachable: quiet centered copy (keel Gate `Failed`),
            ...
            GatePhase::Failed(error) => div()
                ...
                .into_any_element(),
            // A local runtime should not require an app account.
            _ => div()
                .w(px(360.0))
                .px(px(32.0))
                .py(px(40.0))
                ...
                .child(
                    div()
                        ...
                        .font_weight(gpui::FontWeight::SEMIBOLD)
                        .text_color(theme.text)
                        .child(SharedString::from("Local workspace unavailable")),
                )
                .child(
                    div()
                        .mt(px(6.0))
                        ...
                        .text_color(theme.text_muted)
                        .child(SharedString::from(
                            "Keel could not start its local workspace. Retry the local engine.",
                        )),
                )
                .child(
                    div()
                        .id("retry-local-engine")
                        .w_full()
                        .h(px(36.0))
                        .flex()
                        .items_center()
                        ...
                        .on_click(cx.listener(|this, _, _, cx| this.retry_engine(cx)))
                        .child(SharedString::from("Retry local engine")),
                )
                .into_any_element(),
        };
        div()
            ...
            .child(
                div()
                    ...
                    .items_center()
                    .justify_center()
                    // Keyed per phase (keel App.tsx `<div key={phase}
    ...
    fn render_org_gate(&mut self, cx: &mut Context<Self>) -> AnyElement {
        ...
        // keel App.tsx OrgGate: w-400 card on the grid — logo, headline,
        ...
        let blurb: SharedString = match email {
            Some(email) => format!(
                "Keel is organized around workspaces — create one for yourself or your team. Signed in as {email}."
            )
            .into(),
            None => {
                "Keel is organized around workspaces — create one for yourself or your team."
                    .into()
            }
        };
        let card = div()
...
/// The sign-in gate's faint grid backdrop (keel styles.css `.bg-grid`):
...
fn overlay_action_ring(theme: &Theme, selected: bool, action: impl IntoElement) -> gpui::Div {
    div()
        .p(px(1.0))
        .rounded(px(10.0))
        .border_1()
...
/// A size-6 icon button for the titlebar strip (keel window-controls.tsx:
...
fn window_control_button(
    ...
    div()
        ...
        .rounded(px(6.0))
        .cursor_pointer()
        // keel window-controls.tsx: `transition-colors` — the wash fades.
        .bg(motion::hover_blend(
            &fade_key,
            theme.glass_hover().opacity(0.0),
            theme.glass_hover(),
...
/// A titlebar history button (keel window-controls.tsx): enabled it is a
...
/// A size-7 icon button for the main-panel header (keel __root.tsx:
...
fn header_icon_button(
    ...
    div()
        ...
        .rounded(px(6.0))
        .cursor_pointer()
        // keel __root.tsx header buttons: `transition-colors`.
        .bg(motion::hover_blend(
            &fade_key,
            crate::theme::wash(0.0),
            crate::theme::wash(0.11),
...
impl Render for Shell {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        let theme = Theme::of(cx);
        // The shell tone (keel `.frost`): the surface the sidebar sits on and
        ...
        self.sync_flow = sync_flow_after_auth(
            self.sync_flow,
            workspace_scope,
            auth.as_ref(),
            self.boot.workos_client_id.is_some(),
        ...
        if self.focus_sub.is_none() {
            self.focus_sub = Some(cx.on_focus_lost(window, |this: &mut Shell, window, cx| {
                if matches!(
                    this.intro,
                    IntroPhase::Animating | IntroPhase::ReadyForAction
                ) || (this.intro == IntroPhase::Dismissed
                    && !this.settings.jev_onboarding_complete)
                    || this.route == Route::Settings(SettingsSection::Onboarding)
                {
        ...
        let root = div()
            ...
            // no-ops (keel __root.tsx gates the hotkey on `!isSettings`, and
        ...
        let root = match &render_gate {
            GatePhase::Ready => {
                ...
                // Capture knob: `KEEL_OPEN_DIALOG=model` pops the combined
                ...
                // never renders it (keel __root.tsx `!isSettings && activeChat`
                ...
                let card: AnyElement = div()
                    ...
                    .child(main)
                    .into_any_element();
                // The app page fades in after the local engine connects.
            ...
            GatePhase::Loading => {
                let theme = Theme::of(cx).clone();
                root.child(loaders::boot_stage(&theme, cx.entity_id(), cx))
            }
            GatePhase::OrgGate => {
                let card = self.render_gate_card(&GatePhase::SignIn, cx);
                root.child(card)
            }
        ...
        let root = if !restart_required
            ...
            && !self.settings.jev_onboarding_complete
        {
            root.child(self.render_local_onboarding(cx))
        } else {
            root
        };
        ...
        let root = if !restart_required
            ...
            && self.intro != IntroPhase::Dismissed
        {
            root.child(self.render_intro(cx))
        } else {
            root
        };

        // Caption controls are shell-level chrome, not Ready-page content:
        // keep them above every auth/org/error gate as well as
...
mod tests {
    ...
    async fn signed_out_synced_runtime_stops_and_reboots_local() {
        ...
        let boot = EngineBootConfig {
            ...
            default_harness: keel_proto::HarnessId::Mock,
        };
    ...
    #[test]
    fn local_only_auth_does_not_offer_cloud_sync() {
        // Auth::new reports SignedIn for a local-only runtime even though no
        // WorkOS client is configured. It must never open a sync/restart flow.
        let local_auth = AuthState::SignedIn {
            user: keel_proto::UserProfile {
                id: "local-user".into(),
                email: "local-user".into(),
                name: None,
            },
            org_id: None,
        };
        for flow in [
            ...
            assert_eq!(
                sync_flow_after_auth(flow, Some(WorkspaceScope::Local), Some(&local_auth), false),
                SyncFlow::Idle,
            );
        }
    }
    ...
    fn switch_lifecycle_survives_the_runtime_replacement_window() {
        ...
        for flow in [
            ...
            assert_eq!(
                sync_flow_after_auth(flow, Some(WorkspaceScope::Local), Some(&signed_in), true),
                flow
            );
            assert_eq!(sync_flow_after_auth(flow, None, None, true), flow);
            assert_eq!(
                sync_flow_after_auth(flow, Some(WorkspaceScope::Synced), Some(&signed_in), true),
                flow
            );
        }
    }
    ...
    #[test]
    fn titlebar_cluster_matches_keel_window_controls() {
        // keel window-controls.tsx: `left: fullscreen ? 12 : 88` — the
    ...
    // ---- per-session panel flags (§1.10/1.11 parity: keel sessionPanels) ----
    ...
    #[test]
    fn onboarding_is_reachable_from_settings_nav() {
        assert!(SettingsSection::ALL.contains(&SettingsSection::Onboarding));
        assert_eq!(SettingsSection::Onboarding.label(), "Onboarding");
    }
    ...
    fn nav_push_truncates_the_forward_branch() {
        ...
        // semantics — keel's memory history PUSH truncates entries ahead).
        let mut nav = NavHistory::new(chat("a"));
        nav.push(chat("b"));
        nav.push(chat("c"));
        nav.back();
