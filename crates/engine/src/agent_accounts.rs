...
//! (feature-inventory §3.7 "Agent accounts"; port of keel's `agent-accounts.ts`).
//!
...
//! (`/usage` in Claude Code, `/status` in Codex). Unlike keel (fetch on every
...
/// One saved login (`{slotId}.json`), same field surface as keel's slot files.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
struct Slot {
    id: String,
...
impl AgentAccounts {
    ...
    /// Lazy TTL sweep (keel uses a background fiber; native reaps on the next
