...
use keel_harness::{Harness, HarnessError, mock::MockHarness};
use keel_proto::{AgentEvent, DoneStatus, HarnessId, ReasoningLevel, SteeringMode};
...
/// `claude-code` slot resolved through `keel_harness` on first use (subprocess
...
/// Production registry with the local decision preference directory.
pub fn default_registry_for_data_dir(data_dir: &Path) -> HarnessRegistry {
    build_default_registry(Some(data_dir.to_path_buf()))
}

fn build_default_registry(decision_data_dir: Option<PathBuf>) -> HarnessRegistry {
    ...
    keel_harness::shell_env::prewarm();
    ...
    registry.register_lazy(
        ...
        Box::new(|| keel_harness::AcpHarness::claude().installed()),
        Box::new(|| Ok(Arc::new(keel_harness::AcpHarness::claude()) as Arc<dyn Harness>)),
    );
    ...
    // keel_harness::codex::catalog. CLI discovery only happens when a
    ...
    registry.register_lazy(
        ...
        Box::new(|| keel_harness::AcpHarness::codex().installed()),
        Box::new(|| Ok(Arc::new(keel_harness::AcpHarness::codex()) as Arc<dyn Harness>)),
    );
    ...
    registry.register_lazy(
        ...
        Box::new(|| keel_harness::AcpHarness::cursor().installed()),
        Box::new(|| Ok(Arc::new(keel_harness::AcpHarness::cursor()) as Arc<dyn Harness>)),
    );
    ...
    registry.register_lazy(
        ...
        Box::new(|| keel_harness::AcpHarness::grok().installed()),
        Box::new(|| Ok(Arc::new(keel_harness::AcpHarness::grok()) as Arc<dyn Harness>)),
    );
    ...
    registry.register_lazy(
        ...
        Box::new(|| keel_harness::AcpHarness::hermes().installed()),
        Box::new(|| Ok(Arc::new(keel_harness::AcpHarness::hermes()) as Arc<dyn Harness>)),
    );
    ...
    registry.register_lazy(
        ...
        Box::new(|| keel_harness::AcpHarness::pi().installed()),
        Box::new(|| Ok(Arc::new(keel_harness::AcpHarness::pi()) as Arc<dyn Harness>)),
    );
    registry
}
