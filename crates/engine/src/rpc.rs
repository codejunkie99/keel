...
use keel_doc::{MessagePart, SessionCommandPayload};
use keel_proto::{ChatConfig, EngineInfo, HarnessId, ToolCall, WorkspaceScope};
use keel_rpc::{LinkCache, RpcError, RpcReply, RpcService, methods, parse_params};
...
enum MutateParams {
    ...
    /// Full-config replace on the chat row (keel `SetChatConfig`): the
...
pub struct EngineRpc {
    ...
    updater: Option<keel_update::Updater>,
...
impl EngineRpc {
    ...
    pub fn with_updater(mut self, updater: keel_update::Updater) -> Self {
    ...
    fn updater(&self) -> Result<&keel_update::Updater, RpcError> {
        self.updater
            .as_ref()
...
/// The transcript watch as delta frames (`keel_doc::transcript_delta`): a
...
fn doc_messages_stream(
    rx: watch::Receiver<Vec<keel_doc::SessionMessageEntry>>,
) -> BoxStream<'static, serde_json::Value> {
    use keel_doc::transcript_delta::{TranscriptFrame, diff_transcript};
    futures::stream::unfold(
        (rx, None::<Vec<keel_doc::SessionMessageEntry>>),
...
impl RpcService for EngineRpc {
    async fn handle(&self, method: &str, params: serde_json::Value) -> Result<RpcReply, RpcError> {
        ...
        match method {
            ...
            methods::SYNC_STATUS => {
                fn room_json(s: &keel_sync::RoomStatsSnapshot) -> serde_json::Value {
                    serde_json::json!({
            ...
            methods::GET_CHECKOUT_DIFF => {
                ...
                Box::pin(async move {
                    ...
                    .map_err(|e| RpcError::Failed(e.to_string()))?;
                    RpcReply::value(&keel_proto::CheckoutDiff {
            ...
            methods::GET_CHECKOUT_FILE_DIFF_TEXT => {
                ...
                Box::pin(async move {
                    let p: keel_proto::GetCheckoutFileDiffTextRequest = parse_params(params)?;
                    let identity =
                    ...
                    let stale = || keel_proto::CheckoutFileDiffText {
                    ...
                    RpcReply::value(&keel_proto::CheckoutFileDiffText {
