...
//!
//! Pragmatic port of keel's `sessions.ts` (spec: feature-inventory §3.2):
//! - every durable `AgentEvent` is (a) appended to the on-disk run journal, (b) broadcast to
...
//! - native selector activity only updates live session status; it is never journaled or folded;
...
//!
//! Scope notes: sessions are keyed by chat id (one live run per chat). Keel's pulse
...
use keel_harness::{CancellationToken, Harness, RunControls, SteerMessage};
use keel_proto::{
    AgentEvent, DecisionActivity, DecisionBackend, DecisionPhase, DoneStatus, HarnessId,
    RunRequest, Session, SessionStatus, UserInputAnswer, UserInputQuestion,
};

use crate::decision_mode::{self, DecisionMode};
...
/// directory — keel sessions.ts:563 "harness session stores are keyed by
...
struct Inner {
    ...
    /// (keel kept the same pair on `chats.harness_session_id`). An empty
    ...
    /// Decision mode is read at each new task boundary so onboarding can
    /// change it without restarting the engine.
    decision_data_dir: OnceLock<PathBuf>,
    /// Keep the local model warm across task launches. A missing worker is not
    /// cached, so a later install can be recognized without restarting.
    laya_selector: OnceLock<LayaSelector>,
}
...
/// Clears a transient selector indicator even when the dispatch future is
/// cancelled while a model request is in flight.
struct SelectionActivityGuard<'a> {
    inner: &'a Inner,
    chat_id: &'a str,
    active: bool,
}

impl<'a> SelectionActivityGuard<'a> {
    fn new(inner: &'a Inner, chat_id: &'a str) -> Self {
        Self {
            inner,
            chat_id,
            active: false,
    ...
    fn update(&mut self, active: bool, backend: DecisionBackend) {
        if self.active == active {
            return;
        }
        self.active = active;
        self.inner.set_decision_activity(
            self.chat_id,
            active.then_some(DecisionActivity {
                backend,
                phase: DecisionPhase::Choosing,
            }),
        );
    }
}

impl Drop for SelectionActivityGuard<'_> {
    fn drop(&mut self) {
        if self.active {
            self.inner.set_decision_activity(self.chat_id, None);
        }
    }
}
...
impl SessionsEngine {
    pub fn new(
        ...
        Self {
            inner: Arc::new(Inner {
                ...
                decision_data_dir: OnceLock::new(),
                laya_selector: OnceLock::new(),
            }),
        }
    }
    ...
    pub fn set_decision_data_dir(&self, path: PathBuf) {
        let _ = self.inner.decision_data_dir.set(path);
    }

    /// Persist one completed decision boundary as a chronological transcript
    /// receipt. Call this after host validation and outcome observation; the
    /// transient `DecisionActivity` remains the in-flight indicator.
    pub fn record_decision_event(
        &self,
        chat_id: &str,
        event: keel_proto::DecisionEvent,
    ) -> Result<(), EngineError> {
        let host = self
            .inner
            .doc_host()
            .ok_or_else(|| EngineError::Other("doc host is not wired".into()))?;
        host.open(chat_id)?.write_decision_event(event)?;
        Ok(())
    }

    async fn choose_decision_route(
        &self,
        chat_id: &str,
        request: &RunRequest,
    ) -> crate::jev_routing::RouteDecision {
        let skipped = || crate::jev_routing::RouteDecision {
            choice: None,
            event: None,
        };
        let Some(data_dir) = self.inner.decision_data_dir.get() else {
            return skipped();
        };
        ...
        match decision_mode::read(data_dir) {
            ...
            DecisionMode::Jev => {
                ...
                crate::jev_routing::choose_with_report(
                    &self.inner.registry,
                    request,
                    crate::jev_routing::RouteBackend::TypeSafeJev(&key_path),
                    |active| activity.update(active, DecisionBackend::Jev),
                )
                .await
            }
            DecisionMode::Laya => {
                ...
                if self.inner.laya_selector.get().is_none() {
                    ...
                    let _ = self.inner.laya_selector.set(selector);
                }
                let Some(selector) = self.inner.laya_selector.get() else {
                    return skipped();
                };
                crate::jev_routing::choose_with_report(
                    &self.inner.registry,
                    request,
                    crate::jev_routing::RouteBackend::LocalLaya(selector),
                    |active| activity.update(active, DecisionBackend::Laya),
                )
                .await
            }
        }
    ...
    pub fn any_active(&self) -> bool {
        lock(&self.inner.statuses).values().any(|s| {
            matches!(
                s.status,
                keel_proto::SessionStatus::Working | keel_proto::SessionStatus::AwaitingInput
            )
        })
    }
    ...
    ///   (keel's persistent-session routing); otherwise any live run is interrupted
    ...
    async fn dispatch_inner(
        ...
        // A typed decision operates at this clean boundary only. A live steer,
        // resumed provider session, and startup retry retain their route.
        // The selected ID is looked up in the host-owned registry candidate
        // set; it never becomes a command, permission, or tool argument.
        let mut route_event = None;
        if !startup_retry
            && request.resume.is_none()
            && self.inner.resume_for(chat_id, &request.cwd).is_none()
        {
            let decision = self.choose_decision_route(chat_id, &request).await;
        ...
        let user_at = now_ms();
        handle.write_user_message(&user_id, &request.prompt, user_at)?;
        ...
        if request
            ...
            && let Some(workspace) = self
                .inner
                .doc_host()
                .and_then(|host| host.workspace().cloned())
        {
            let config = keel_proto::ChatConfig {
                harness: harness_id,
            ...
            if let Err(error) = workspace.set_chat_config(chat_id, &config) {
                tracing::warn!(chat = %chat_id, error = %error, "failed to persist Jev route");
            }
        }

        // Engine-owned resume (keel sessions.ts:736 — every dispatch read the
        ...
        // These keys configure Keel's host scheduler, not the coding CLI's
        // model options. Keep them in the persisted request for continuation,
        // then remove them from the actual provider request.
        request.model_options.remove("jevAuto");
        request.model_options.remove("jevRouted");
        ...
        if let Some(mut event) = route_event {
            event.created_at = now_ms().max(user_at.saturating_add(1));
            if event.observed_outcome.is_none() {
                event = event.with_observed_outcome("Selected coding route scheduled");
            }
            if let Err(error) = self.record_decision_event(chat_id, event) {
                tracing::warn!(chat = %chat_id, error = %error, "could not persist route decision");
            }
        }
        Ok(run_id)
    }
    ...
    pub async fn interrupt(&self, chat_id: &str) -> Result<bool, EngineError> {
        ...
        // Unpark any blocked question FIRST (mirrors keel: harness teardown can await a
    ...
    /// the remembered harness session (keel: "not just eulogized";
    ...
    pub fn recover_stale(&self) -> Result<usize, EngineError> {
        ...
        for chat_id in stale {
            ...
            // same harness conversation (keel recoverDraft, sessions.ts:538).
            ...
            tokio::spawn(async move {
                ...
                let request = sessions
                    ...
                    // Last resort: the journal's own cwd (keel's draft config)
...
impl Inner {
    fn set_decision_activity(&self, chat_id: &str, activity: Option<DecisionActivity>) {
        let now = Utc::now();
        let session = {
            ...
            let entry = statuses
                ...
                .or_insert_with(|| Session {
                    ...
                    started_at: Some(now),
                    updated_at: now,
                    decision_activity: None,
                });
            if activity.is_some() && !matches!(entry.status, SessionStatus::Working) {
                entry.status = SessionStatus::Working;
                entry.started_at = Some(now);
            }
            entry.decision_activity = activity;
    ...
    fn set_status(&self, chat_id: &str, status: SessionStatus, fresh_start: bool) {
        ...
        let session = {
            ...
            entry.status = status;
            if matches!(status, SessionStatus::Idle | SessionStatus::Errored) {
                entry.decision_activity = None;
            }
            entry.updated_at = now;
            match status {
    ...
    /// engine restart (keel sessions.ts:1039).
...
#[cfg(test)]
mod decision_activity_tests {
    use super::*;

    #[test]
    fn selector_activity_is_broadcast_and_cleared_on_drop() {
        let dir = tempfile::tempdir().unwrap();
        let journal = Arc::new(RunJournal::open(dir.path()).unwrap());
        let registry = Arc::new(HarnessRegistry::new());
        let engine = SessionsEngine::new("device".into(), journal, registry);
        let receiver = engine.watch_sessions();
        {
            let mut guard = SelectionActivityGuard::new(&engine.inner, "chat");
            guard.update(true, DecisionBackend::Jev);
            assert_eq!(
                receiver.borrow()[0].decision_activity,
        ...
        assert_eq!(receiver.borrow()[0].decision_activity, None);
    }

    #[test]
    fn native_focus_activity_is_live_only_and_done_clears_it() {
        let dir = tempfile::tempdir().unwrap();
        let journal = Arc::new(RunJournal::open(dir.path()).unwrap());
        ...
        let receiver = engine.watch_sessions();
        let activity = DecisionActivity {
            backend: DecisionBackend::Laya,
            phase: DecisionPhase::ChoosingFocus,
        };
        let event = AgentEvent::DecisionActivity {
            activity: Some(activity),
        };
        assert!(consume_live_decision_activity(
            &engine.inner,
            "chat",
            HarnessId::Mock,
            &event
        ...
        assert_eq!(receiver.borrow()[0].decision_activity, Some(activity));
        let mut parts = Vec::new();
        fold_event_into_parts(&mut parts, &event);
        assert!(parts.is_empty());
        assert!(journal.replay("chat", 0).unwrap().is_empty());

        engine.inner.set_status("chat", SessionStatus::Idle, false);
        assert_eq!(receiver.borrow()[0].decision_activity, None);
    }
}
...
/// Consume the native selector's transient status before journal and doc work.
fn consume_live_decision_activity(
    inner: &Inner,
    chat_id: &str,
    harness_id: HarnessId,
    event: &AgentEvent,
...
async fn drive_run(
    ...
    // PERSISTENT SESSION (keel runsBySession): a completed turn on a
    ...
    // a session nobody comes back to (keel SESSION_IDLE_MS).
    ...
    // moment output flows again, and nothing is lost. `KEEL_TURN_QUIESCE_MS`
    // overrides the window; 0 disables.
    let quiesce_after: Option<std::time::Duration> = match std::env::var("KEEL_TURN_QUIESCE_MS")
        .ok()
    ...
    // push the window during real thinking. `KEEL_SELF_TURN_QUIESCE_MS`
    // overrides; 0 falls back to the normal window. An explicit
    // `KEEL_TURN_QUIESCE_MS=0` still disables the watchdog entirely.
    let self_quiesce_after: Option<std::time::Duration> =
        match std::env::var("KEEL_SELF_TURN_QUIESCE_MS")
            .ok()
            .and_then(|v| v.parse::<u64>().ok())
        {
            Some(0) => None,
    ...
    let final_status = loop {
        let event: AgentEvent = tokio::select! {
            ...
            // Idle reaper (keel SESSION_IDLE_MS): a parked persistent session
            ...
            _ = tokio::time::sleep_until({
                ...
                && !folded.iter().any(|p| match p {
                    MessagePart::Tool { id, resolved: false, .. } => {
                        id != keel_proto::LIVE_PLAN_TOOL_ID
                    }
        ...
        if consume_live_decision_activity(&inner, &chat_id, harness_id, &event) {
            continue;
        }
            ...
            let self_continued = idle_since
                ...
                && (matches!(
                    ...
                    AgentEvent::ToolCall { id, .. }
                        if id == keel_proto::LIVE_PLAN_TOOL_ID || !seen_tools.contains(id)
                ));
            if self_continued {
                tracing::info!(
                    chat = %chat_id,
        ...
        match &event {
            ...
            AgentEvent::ToolCall { id, .. } if id == keel_proto::LIVE_PLAN_TOOL_ID => {}
            AgentEvent::ToolResult { id, .. } if id == keel_proto::LIVE_PLAN_TOOL_ID => {}
            AgentEvent::ToolCall { id, .. } => {
        ...
        // Defensive rule from keel: a mid-run SessionStarted re-emission (Claude SDK
        ...
        if !skip_fold {
            ...
            // journal. To reintroduce: `keel_doc::sidecar_payload(&event)`
