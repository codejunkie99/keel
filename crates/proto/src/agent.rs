...
use crate::entities::{DecisionActivity, DecisionEvent};
...
pub struct RunRequest {
    ...
    /// refs (keel's `withAttachments` transport — that's what persists in the
...
/// A provider capability observed through ACP discovery. Advertisement is not
/// proof that Keel can execute the capability: most ACP tools remain owned by
/// the provider's internal loop.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct CapabilityInventory {
    pub provider: HarnessId,
    /// The short-lived probe session that produced session-scoped entries.
    /// Consumers must revalidate those entries against the active run.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub observed_session_id: Option<String>,
    #[serde(default)]
    pub capabilities: Vec<ProviderCapability>,
    /// ACP does not give Keel an operation for dispatching the provider's
    /// internal tools, even when the provider advertises their names.
    #[serde(default)]
    pub provider_owned_tools_opaque: bool,
}

impl CapabilityInventory {
    pub fn empty(provider: HarnessId) -> Self {
        Self {
            provider,
            observed_session_id: None,
            capabilities: Vec::new(),
            provider_owned_tools_opaque: true,
        }
    }
}
...
pub struct ProviderCapability {
    /// Stable within a provider snapshot, for example `slash:compact`.
    pub id: String,
...
pub enum CapabilityInvocation {
    /// ACP supplies metadata only; `/name` is still ordinary prompt text.
    PromptText,
    SessionConfig {
        config_id: String,
        value: String,
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub enum CapabilityPrecondition {
    ActiveSession,
    RevalidateAdvertisement,
    HostAuthorization,
}
...
///
/// Mirrors keel's `AgentEvent` tagged enum.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
...
pub enum AgentEvent {
    ...
    /// A completed host-owned decision boundary. The payload is a bounded,
    /// factual receipt and must not contain model reasoning or raw context.
    Decision {
        event: DecisionEvent,
    },
    /// Live selector state for the in-process harness. The engine consumes
    /// this before journaling or folding; it is never a chat event.
    DecisionActivity {
        activity: Option<DecisionActivity>,
    },
...
mod tests {
    ...
    #[test]
    fn capability_inventory_round_trips_without_upgrading_advertisements() {
        let inventory = CapabilityInventory {
            provider: HarnessId::Codex,
            observed_session_id: Some("probe-1".into()),
            provider_owned_tools_opaque: true,
            capabilities: vec![ProviderCapability {
                id: "slash:compact".into(),
                ...
                invocation: CapabilityInvocation::PromptText,
                input_hint: None,
                preconditions: vec![CapabilityPrecondition::RevalidateAdvertisement],
                effect: CapabilityEffect::ProviderDefined,
            }],
        };
        let value = serde_json::to_value(&inventory).unwrap();
        let round: CapabilityInventory = serde_json::from_value(value).unwrap();
        assert_eq!(round, inventory);
        assert_eq!(round.capabilities[0].status, CapabilityStatus::Advertised);
    }
}
